...
const ApplyJob = () => {
    ...
    const toggleSaveJob = async () => {
        if (!user) return toast.info("Please login to save jobs.");
        try {
    ...
    if (!job) {
        return (
            <Container className="py-5 text-center">
                <h2>Job not found</h2>
                <Button variant="primary" onClick={() => navigate('/jobs')} className="mt-3">Back to Jobs</Button>
            </Container>
        );
    }
                        ...
                        <Card className="border-0 shadow-sm rounded-4 mb-4">
                            <Card.Body className="p-4 p-md-5">
                                ...
                                <p className="text-muted fs-5 mb-4">
                                    <span className="text-primary fw-bold">{job.employer?.name || 'Company Name'}</span> • {job.location} • Posted {new Date(job.createdAt).toLocaleDateString()}
                                </p>
                                ...
                                {user && matchScore !== null && (
                                    <Alert variant={matchScore >= 70 ? 'success' : matchScore >= 40 ? 'warning' : 'danger'} className="d-flex align-items-center rounded-3 border-0">
                                        ...
                                        <div>
                                            <div className="fw-bold mb-1">AI Match Score: {matchScore}%</div>
                                            <ProgressBar 
                                                now={matchScore} 
                                ...
                                <div className="mt-5">
                                    <h4 className="fw-bold mb-3 border-bottom pb-2">Job Description</h4>
                            ...
                            <Card className="border-0 shadow-sm rounded-4">
                                <Card.Body className="p-4">
                                    ...
                                    <div className="d-flex align-items-center mb-3">
                                        ...
                                        <span className="fw-bold fs-5">{job.employer?.name || 'Company Name'}</span>
                                    </div>
                                    ...
                                    <div className="text-muted small">
                                        Member since {new Date(job.employer?.createdAt || Date.now()).getFullYear()}
                                    </div>
                                </Card.Body>
                            </Card>
                        </div>
